  ...
  return (
    <aside className="floating-cart-shell">
      ...
      >
        ...
        <span className="cart-badge min-w-[3.25rem] justify-center px-2">{isOpen ? "-" : itemsCount}</span>
      </button>
      ...
      {isOpen ? (
        <div id="floating-cart-panel" className="floating-cart-panel mt-3">
            ...
            {cart.map((item) => (
              <div key={item.product.id} className="mini-product-card">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="section-label text-sm tracking-[0.2em]">{item.product.category}</p>
                ...
                <div className="mt-3 flex items-center justify-between gap-3">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#8f8f8f]">Subtotal</p>
                ...
                <p className="mt-2 text-xs text-[#b8b8b8]">{formatCurrency(item.product.price)} por unidad</p>
              </div>
            ))}
          </div>
          ...
          <button
            type="button"
            onClick={onCheckout}
            className="neon-button mt-4 w-full border border-[var(--neon-gold)] bg-[var(--neon-gold)] px-6 py-4 font-black tracking-[0.22em] text-black hover:border-[var(--neon-gold-true)] hover:bg-[var(--neon-gold-true)] hover:shadow-[0_0_40px_var(--neon-gold)]"
          >
            Confirmar pedido por WhatsApp
          </button>
          <p className="mt-3 text-center text-xs font-bold text-[var(--neon-gold)]">
